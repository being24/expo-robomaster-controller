...
constexpr float TEST_TARGET_RPM = 8.0f;
constexpr float MAX_TEST_CURRENT_A = 1.0f;
constexpr float CURRENT_SLEW_RATE_A_PER_SEC = 0.1f;
...
float slewCurrent(float current, float desired, float elapsedSeconds) {
  const float maxChange = CURRENT_SLEW_RATE_A_PER_SEC * elapsedSeconds;
  if (desired > current) return min(current + maxChange, desired);
  if (desired < current) return max(current - maxChange, desired);
  return current;
}
...
void sendCurrent(float currentA) {
  ...
  currentA = constrain(currentA, -MAX_TEST_CURRENT_A, MAX_TEST_CURRENT_A);
...
void loop() {
  ...
  if (now - lastCommandMs >= COMMAND_INTERVAL_MS) {
    const float elapsedSeconds =
        lastCommandMs == 0 ? 0.0f : (now - lastCommandMs) / 1000.0f;
    const float desiredCurrent =
        hasFeedback ? constrain(rpmToCurrent(targetRpm), 0.0f, MAX_TEST_CURRENT_A)
                    : 0.0f;

    outputCurrent = slewCurrent(outputCurrent, desiredCurrent, elapsedSeconds);
