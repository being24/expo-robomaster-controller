...
M5Stack AtomS3とUnit CANを使用したRoboMasterモーター制御システム。CANバス通信とPID制御により、外部からのUDPコマンドでモーターの回転速度を制御します。
...
# .vscode/arduino.json の port に書き込み
./bin/write.sh
```
...
2. **CAN通信が確立しない**
   - Unit CANがAtomS3のPort A (TX: GPIO2、RX: GPIO1) に接続されていることを確認
   - モーターの電源とCAN終端抵抗を確認
